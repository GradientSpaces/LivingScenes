...
class DGCNN(nn.Module):
    def __init__(
        ...
        if use_bn:
            ...
            self.conv5 = nn.Sequential(nn.Conv2d(512, self.emb_dims, kernel_size=1, bias=False), self.bn5, nn.LeakyReLU(negative_slope=0.2))
            ...
            self.conv5 = nn.Sequential(nn.Conv2d(512, self.emb_dims, kernel_size=1, bias=False), nn.LeakyReLU(negative_slope=0.2))

        self.head_centroid = nn.Sequential(nn.Linear(self.emb_dims, 3))
        self.head_scale = nn.Sequential(nn.Linear(self.emb_dims, 1))
    def forward(self, x):
        ...
        global_feat = point_feat.max(dim=2, keepdim=False)[0]       # global feat: batch x emb_dim
        # center_pred, pred_scale, pred_so3_feat, pred_inv_feat

        scale = self.head_scale(global_feat).squeeze()
        # center_pred = torch.zeros((batch_size, 3)).cuda().float()
